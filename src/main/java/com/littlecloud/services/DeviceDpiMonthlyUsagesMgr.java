...
public class DeviceDpiMonthlyUsagesMgr {
	...
	public int doDeviceDpiMonthlyUsagesConsolidation(Calendar calFrom, Calendar calTo){
		int noOfRecordsDone = 0;
		try{
			noOfRecordsDone = doDeviceDpiMonthlyUsagesConsolidation(calFrom, calTo, null);
		} catch (Exception e){
	...
	public int doDeviceDpiMonthlyUsagesConsolidation(Calendar calFrom, Calendar calTo, List<Networks> networkList){
		int noOfRecordsDone = 0;
		try{
			if (networkList == null){
					...
					if( deviceMonthlyDpiUsageList != null ){
						...
						if (deviceMonthlyDpiUsageSaveList != null && deviceMonthlyDpiUsageSaveList.size() > 0){
							...
							if (!areSaved){
								...
								noOfRecordsDone = deviceMonthlyDpiUsageSaveList.size();
							}
						}
