...
public class NetworkUtils 
{
	public static enum FEATURE {
		wifi, map, dpi, mastermode, portal_ic2;
	}
	...
	public static boolean updateNetworksFeature(String orgId, Integer netId)
	{
		...
		try
		{
			NetworksMgr netMgr = new NetworksMgr(orgId);
			...
			Networks network = netMgr.findById(netId);
			if( network != null )
			{
				String net_feature = "";
				...
				List<Devices> devices = devicesDAO.getDevicesList(netId);
				if( devices != null )
				{
...
//					}
				...
				netMgr.update(network);
			}
			else
			{
				log.warnf("network %d is null", netId);
			}
	...
	public static boolean isGpsTrackingDisabled(String orgId, Integer netId)
	{
		...
		if (orgId != null && netId != null) 
		{
			...
			if (networks != null && networks.isGps_tracking_disabled())
				support = true;
			/* Attention: For type Boolean, if Gps_tracking_disabled() = null, "networks.getGps_tracking_disabled() == true" caused NullPointerExecption !! */
			return support;
			
			} catch (NullPointerException en) {
				log.errorf("NullPointerException - Fail to get isGpsTrackingDisabled for orgId %s dev %d %s", orgId, netId, en);
				
			}catch (Exception e) {
				log.errorf("Exception - Fail to get isGpsTrackingDisabled for orgId %s dev %d %s", orgId, netId, e);
