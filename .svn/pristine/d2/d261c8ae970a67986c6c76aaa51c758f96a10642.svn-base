...
public class CommonUtils {
		...
		try{
			if (isFileExisted(filePath)){
		        ...
		        if (length > Integer.MAX_VALUE) {
		            log.warnf("File: %s - length is too large, larger than integer max value!", filePath);
		        }
				...
				log.warnf("CommonUtils.readFile2ByteArray() - filePath: %s is not existed", filePath);
			}
		} catch (Exception e){
	...
	public static String genOrgTimestampString(String orgId){
		...
		try{
			...
			log.error("CommonUtils.genOrgTimestampString() - ", e);
		}
		return rtnStr;
	}
...
//		} catch (Exception e){
//			log.error("CommonUtils.convertCalendar2MySqlDateString() - ", e);
//		}
//		return mySqlDateString;
//	}
	...
	public static int safeLongToInt(long l) {
		try{
			...
			if (log.isDebugEnabled()){
				log.debug("CommonUtils.safeLongToInt() - cannot be cast to int without changing its value.");
			}
		}
	    return (int) l;
	}
	public static float safeDoubleToFloat(double d) {
		try{
			...
			if (log.isDebugEnabled()){
				log.debug("CommonUtils.safeDoubleToFloat() - cannot be cast to float without changing its value.");
			}
		}
	    return (float) d;
	}
